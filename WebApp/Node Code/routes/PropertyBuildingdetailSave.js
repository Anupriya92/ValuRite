...
docs.forEach(function(subdocs){	
	if(jsonObj.flag){
		...
		coll.update({'ValuationID':ValuationID,'ValuationStatus':'true'}, {$set : {BuildingStatus : "Retrieved"}});
 		var arr = JSON.stringify(subdocs.BuildingDetails);
        res.send(arr);
	...
	{$set :
	{
		"BuildingDetails" : JSON.parse(jsonObj.BuildingDetails),
