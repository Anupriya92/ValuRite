	...
	try {
		...
		collection.aggregate(query, function (err, docs) {
			...
			if(err) {
				...
				return res.status(res.statusCode).send("There was a problem finding the user.");
			} else {
			if (docs == "" || docs == undefined) {
						obj = { status: 'Not Exists' , 
							...
							{ multi: true }, function (err2, docs2) {
								...
								if (err2) console.warn(err.message);
									...
									logcollection.insert(
										...
										obj2 = {session_id: session_id, name: doc.first_name, rolename: doc.rolename, companyname: doc.company_institute, username: doc.email_id,userid:doc.user_id, general_alert_flag: doc.general_alert_flag,userID: doc.user_id,usertype:doc.usertype,status:doc.status,OrgType:doc.OrgType}
									obj = { status: 'Valid',
				...
				else if (doc.updatetime.length == 0)   // || (date.subtract(new Date(), subdoc.updatetime[0].session_update_time).toSeconds() > 15)
				{
					if (doc.max_allowed_logins < doc.logged_in_users) {
						...
						obj2 ={session_id: session_id, name: doc.first_name, rolename: doc.rolename, companyname: doc.company_institute, username: doc.email_id, general_alert_flag: doc.general_alert_flag,userID:doc.user_id,usertype:doc.usertype,status:doc.status, OrgType:doc.OrgType}
						obj = { status: 'Valid',
						Msg : "Given user data are valid - Successfully LoggedIn",
						result : obj2
						 }
