...
export class LoginPageComponent implements OnInit {
  ...
  constructor(private route: Router, public trigger: MenuService, private config: ConfigService) {
  }
  loginpopup :boolean;
  ...
  postPasswordValidation(res, data) {
    ...
    sessionStorage.setItem('OrgType', res.result.OrgType);
  ...
  'usertype':usertype, 'userId': userId, 'firstname':name, 'companyName': company, 'OrgType':OrgType}
  //ar userdata = {'USEsername':username,'us':rolename}
    var urlQuery = jQuery.param(userdata)
    if(usertype != "Admin") {
    var localurl=this.config.valuRiteURL();
     sessionStorage.clear();
        $.ajax({
                async : false,
              // url : 'http://49.207.182.154/PropApprasial/ValuRite/ApplicationQueue.aspx',
                 url: localurl,
                success: function(data){
                  //window.open("http://49.207.182.154/PropApprasial/ValuRite/ApplicationQueue.aspx?" +urlQuery,"_self");
              window.open(localurl+"?"+urlQuery,"_self");
              },
                error: function(data) {
                 alert("Server problem can't navigate. Please try later")
                }
              });
